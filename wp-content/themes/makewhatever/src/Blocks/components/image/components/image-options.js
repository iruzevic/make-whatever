		...
		>
			<Responsive
				...
				options={generateOptionsFromValue(imageData, (v) =>
					truncateMiddle(v?.url?.replace(window.location.origin, '')?.replace(/\/wp-content\/uploads\/\d{4}\/\d{2}\//g, '') ?? __('Not set', 'makewhatever'), 15),
				)}
			...
			<OptionSelect
				...
				label={__('Aspect ratio', 'makewhatever')}
