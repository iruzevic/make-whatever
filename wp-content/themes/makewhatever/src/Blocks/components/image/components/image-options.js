...
export const ImageOptions = (attributes) => {
	...
	if (!hideImagePicker && !imageUrl[breakpoints[0]]) {
		return (
			<UseToggle {...generateUseToggleConfig(attributes, manifest, 'imageUse')}>
				<Control icon={icons.image} label={__('Image', 'makewhatever')} additionalLabelClasses='es-h-spaced' noBottomSpacing>
					<MediaPlaceholder
						labels={{
							title: __('Add an image', 'makewhatever'),
	...
	return (
		<UseToggle {...generateUseToggleConfig(attributes, manifest, 'imageUse')}>
			{!hideImagePicker &&
				<Responsive label={__('Image', 'makewhatever')} icon={icons.image}>
						...
						return (
							<div className='es-h-center es-items-end! es-gap-0!' key={breakpointName}>
								...
								<Button
									key={index}
									icon={icons.trashAlt}
									label={__('Remove image', 'makewhatever')}
			...
			<Section
				...
				icon={icons.design}
				label={__('Design & layout', 'makewhatever')}
			...
			>
				...
				{!hideFullSizeToggle &&
					<IconToggle
						icon={icons.expandXl}
						label={__('Fill container', 'makewhatever')}
						checked={imageFull}
			...
			<Section
				showIf={!hideAltText}
				icon={icons.a11y}
				label={__('Accessibility', 'makewhatever')}
				noBottomSpacing={!additionalControlsAfterA11y}
			>
				<TextControl
					label={<IconLabel icon={icons.altText} label={__('Alt text', 'makewhatever')} />}
					value={imageAlt}
					onChange={(value) => setAttributes({ [getAttrKey('imageAlt', attributes, manifest)]: value })}
					help={__('Describes the content of the image', 'makewhatever')}
