...
export const FeaturedContentOptions = ({ attributes, setAttributes }) => {
	...
	const allGenericOption = { label: __('Show all', 'makewhatever'), value: '_all', separator: 'below' };
	...
	return (
		<>
			...
			<NumberPicker
				...
				label={__('Items per page', 'makewhatever')}
			...
			<OptionSelect
				icon={icons.multiple}
				label={__('Content type', 'makewhatever')}
			...
			<OptionSelect
				aria-label={__('Select items to show', 'makewhatever')}
				...
				options={[
					{ label: __('Show all posts', 'makewhatever'), value: false },
					{
						label: __('Select posts to show', 'makewhatever'),
						value: true,
						disabled: featuredContentExcludeCurrentPost || featuredContentTaxonomy,
						subtitle: (featuredContentExcludeCurrentPost || featuredContentTaxonomy) && __('Unavailable when a taxonomy filter is set', 'makewhatever'),
					},
				]}
			...
			>
				<Toggle
					...
					label={__('Exclude current', 'makewhatever')}
					subtitle={__('Best used with blog posts', 'makewhatever')}
			...
			>
				<AsyncMultiSelect
					...
					help={__('Newest 30 items are shown, others can be selected by searching. If blank, all items are shown.', 'makewhatever')}
			...
			<Toggle
				icon={icons.dice}
				label={__('Randomize order', 'makewhatever')}
			...
			<OptionSelect
				icon={icons.filter}
				label={__('Taxonomy', 'makewhatever')}
			...
			<OptionSelect
				aria-label={__('Select terms to show', 'makewhatever')}
				value={termsToShow}
				onChange={(value) => {
				...
				options={[
					{ label: __('Show all terms', 'makewhatever'), value: 'all' },
					{
						label: __('Select terms to show', 'makewhatever'),
						value: 'manual',
						disabled: (featuredContentTaxonomy ?? allGenericOption)?.value === '_all',
						subtitle: (featuredContentTaxonomy ?? allGenericOption)?.value === '_all' && __('Select a taxonomy to enable', 'makewhatever'),
					},
					{
						label: __('Show same terms as current post', 'makewhatever'),
						value: 'current',
					},
				]}
				type='radiosSegmented'
			...
			>
				<AsyncMultiSelect
					...
					reFetchOnSearch
					placeholder={__('Select terms to show', 'makewhatever')}
				/>
			</AnimatedVisibility>
			...
			<LoadMoreOptions
				...
				label={__('"Load more" button', 'makewhatever')}
				noUseToggle
			/>
		</>
	);
