		...
		<div className='p-8'>
			<div className='mx-auto max-h-[80vh] min-h-[40vh] w-full max-w-[50vw] rounded-lg border border-dashed border-gray-400 p-4'>
				<div className='flex justify-between gap-4'>
					<RichText
						placeholder={__('Modal title', 'makewhatever')}
						value={modalTitle}
					...
					>
						<InputField
							icon={icons.id}
							label={__('Modal unique identifier', 'makewhatever')}
							help={__('Used for opening the modal', 'makewhatever')}
							value={modalId}
				...
				>
					<Notice
						type='warning'
						label={__('Modal unique identifier not set', 'makewhatever')}
					/>
				</AnimatedVisibility>
			</div>
			<AnimatedVisibility
			...
			>
				<Notice
					type='success'
					label={__('Modal is ready to use', 'makewhatever')}
					subtitle={
						<>
							{__('To open, set an URL in any element that accepts it to ', 'makewhatever')}
