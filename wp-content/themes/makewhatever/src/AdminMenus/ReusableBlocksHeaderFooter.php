 ...
 *
 * @package Makewhatever\AdminMenus
 */

declare(strict_types=1);

namespace Makewhatever\AdminMenus;
...
class ReusableBlocksHeaderFooter extends AbstractAdminMenu
{
	...
	{
		return \esc_html__('Header & footer', 'makewhatever');
	}
	...
	public function registerWpSettings(): void
	{
		...
		\add_settings_field(
			self::HEADER_PARTIAL,
			\__('Header partial', 'makewhatever'),
		...
		\add_settings_field(
			self::FOOTER_PARTIAL,
			\__('Footer partial', 'makewhatever'),
			[$this, 'renderPartialSelector'],
			self::ADMIN_MENU_SLUG,
			self::SETTINGS_SECTION_NAME,
			[
	...
	public function renderPartialSelector($args): void
	{
		...
		if ($reusableBlocksQuery->have_posts()) {
			...
			<select id="<?php echo \esc_attr($args['label_for']); ?>" name="<?php echo \esc_attr($type); ?>">
				<option value="">
					&mdash; <?php \esc_html_e('None', 'makewhatever'); ?> &mdash;
				</option>
		...
		<?php } else { ?>
			<i><?php echo \esc_html__('No reusable blocks found.', 'makewhatever'); ?></i>
			<?php
		}
	}
